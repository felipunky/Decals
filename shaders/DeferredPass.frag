...
{             
    ...
    if (iNormals == 1)
    {
        ...
        FragColor = vec4(pow(ambient + diffuse + specular, vec3(0.45454)), 1.0);

        /*float metallic  = 0.0;//texture(gMetallic, TexCoords).r;
        float roughness = 0.0;texture(gRoughness, TexCoords).r;
        float ao        = 0.0;//texture(gAO, TexCoords).r;
        ...
        for(int i = 0; i < 1; ++i) 
        {
             ...
             vec3 light = normalize(TangentLightPos - TangentFragPos);
